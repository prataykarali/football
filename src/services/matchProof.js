...
export class MatchProofService {
  ...
  async engagementFingerprint(engagementData) {
    return this.generateEngagementFingerprint(engagementData);
  }
  ...
  async toCairoCalldata(input) {
    ...
    const fingerprintHex = await this.engagementFingerprint(engagement);
  ...
  async verify(input, expectedProofIdHex) {
    return this.verifyProof(input, expectedProofIdHex);
  }
