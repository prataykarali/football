...
export class NightOwlService {
  ...
  offBigMoment(callback) {
    this.bigMomentCallbacks.delete(callback);
  }
  ...
  getCatchUpSummary(sleepStartTimestamp = 0) {
    const missed = this.eventsList.filter(e => {
      const ts = e.timestamp || Date.now();
      return ts >= sleepStartTimestamp;
    });

    return {
      missedEvents: missed,
      summary: missed.length > 0 ? missed.map(e => `${e.minute}' — ${e.details || e.type}`).join('\n') : 'No missed events.'
    };
  }
  ...
  generateMicroQuiz() {
    ...
    return {
      question: `Did ${lastEvent.player || lastEvent.team} contribute to ${lastEvent.type}?`,
      options: ['Yes', 'No']
    };
  }
