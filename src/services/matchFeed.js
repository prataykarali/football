...
export class MatchFeed {
  constructor(events = [], speed = 10, teams = {}) {
    this.events = events;
    this.speed = speed;
  ...
  setTeams({ homeCode, awayCode } = {}) {
    if (homeCode) this.homeCode = homeCode;
    if (awayCode) this.awayCode = awayCode;
  }
  ...
  _emitEvent(event) {
    ...
    if (event.type === 'goal') {
      if (event.team === this.homeCode) this.score.home++;
      else if (event.team === this.awayCode) this.score.away++;
    }
